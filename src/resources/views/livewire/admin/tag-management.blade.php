<div>
    <div class="flex justify-between items-center mb-6">
        ...
        <button wire:click="createTag" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md">
            @svg('lucide-plus', 'w-5 h-5 inline-block mr-1')
            Add Tag
        </button>
    </div>

    @if($tagId !== null || !$isEditingTag)
        <div class="bg-zinc-800 rounded-lg shadow p-6 mb-6">
            <h3 class="text-lg font-medium mb-4">{{ $isEditingTag ? 'Edit Tag' : 'Create Tag' }}</h3>
            <form wire:submit="saveTag" class="space-y-4">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div>
                        <label for="projectTagName" class="block text-sm font-medium text-gray-400 mb-1">Name</label>
                        <input type="text" id="projectTagName" wire:model="projectTagName" class="bg-zinc-700 border border-zinc-600 text-white block w-full px-3 py-2 rounded-md">
                        @error('projectTagName') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label for="tagIcon" class="block text-sm font-medium text-gray-400 mb-1">Icon</label>
                        <input type="text" id="tagIcon" wire:model="tagIcon" class="bg-zinc-700 border border-zinc-600 text-white block w-full px-3 py-2 rounded-md">
                        ...
                        @error('tagIcon') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label for="tagGroupId" class="block text-sm font-medium text-gray-400 mb-1">Tag Group</label>
                        <select id="tagGroupId" wire:model="tagGroupId" class="bg-zinc-700 border border-zinc-600 text-white block w-full px-3 py-2 rounded-md">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
                        @foreach($projectTypes as $type)
                            <label class="inline-flex items-center">
                                <input type="checkbox" wire:model="tagProjectTypes" value="{{ $type->id }}" class="bg-zinc-700 border-zinc-600 text-indigo-600 rounded">
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button" wire:click="resetTagForm" class="bg-zinc-600 hover:bg-zinc-700 text-white px-4 py-2 rounded-md">
                        Cancel
                    </button>
                ...
                @foreach($tags as $tag)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button wire:click="editTag({{ $tag->id }})" class="text-indigo-400 hover:text-indigo-300 mr-3">
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-zinc-800 rounded-lg max-w-md w-full p-6">
            ...
            <p class="text-gray-300 mb-6">Are you sure you want to delete this tag? This action cannot be undone.</p>
            <div class="flex justify-end space-x-3">
